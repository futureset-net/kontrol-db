...
class ProtectedConnectionTest {
    ...
    @Test
    fun `check cannot commit connection because it is managed by transaction`(@DialectName dialect: String) {
        assumeThat(dialect).isEqualTo("hsqldb")
        KontrolDbEngineBuilder.dsl {
            loadConfig("test-config.yml")
        }.use { engine ->
            val logs = capturingLogEvents {
    ...
    @Test
    fun `check cannot close connection because it is managed by transaction`(@DialectName dialect: String) {
        assumeThat(dialect).isEqualTo("hsqldb")
        KontrolDbEngineBuilder.dsl {
            loadConfig("test-config.yml")
        }.use { engine ->
    ...
    @Test
    fun `check change autocommit state because it is managed by transaction`(@DialectName dialect: String) {
        assumeThat(dialect).isEqualTo("hsqldb")
        KontrolDbEngineBuilder.dsl {
            loadConfig("test-config.yml")
        }.use { engine ->
