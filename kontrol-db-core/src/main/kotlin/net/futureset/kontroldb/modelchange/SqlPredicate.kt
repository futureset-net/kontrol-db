...
interface SqlPredicate : SqlString {
    ...
    operator fun plus(predicate: SqlPredicate): SqlPredicate = if (isEmpty()) predicate else AllOf(listOf(this, predicate))
}
...
data class AllOf(
    val predicates: List<SqlPredicate>,
) : SqlPredicate {
    override fun isEmpty() = predicates.isEmpty()

    override fun plus(predicate: SqlPredicate) = if (predicate.isEmpty()) {
        this
...
data class Not(
    ...
    override fun toQuoted(effectiveSettings: EffectiveSettings): String = "NOT ${predicate.toQuoted(effectiveSettings)}"
}

data class Exists(
    private val selectQuery: SelectQuery,
) : SqlPredicate {
    override fun toQuoted(effectiveSettings: EffectiveSettings): String = "EXISTS (${
...
data class Eq(
    ...
    override fun toQuoted(effectiveSettings: EffectiveSettings) = a.toQuoted(effectiveSettings) + (" = ".takeUnless { b.isNull() } ?: " IS ") + b.toQuoted(effectiveSettings)
}
...
data class Gt(
    ...
    override fun toQuoted(effectiveSettings: EffectiveSettings) = a.toQuoted(effectiveSettings) + " > " + b.toQuoted(effectiveSettings)
}
...
data class Between(
    ...
    override fun toQuoted(effectiveSettings: EffectiveSettings) = subject.toQuoted(effectiveSettings) + " BETWEEN " + a.toQuoted(effectiveSettings) + " AND " +
        b.toQuoted(
            effectiveSettings,
        )
}
...
class PredicateBuilder(
    private var predicate: SqlPredicate = AllOf(predicates = emptyList()),
) : Builder<PredicateBuilder, SqlPredicate>,
    SqlValueFactory {
    fun existsSelect(
