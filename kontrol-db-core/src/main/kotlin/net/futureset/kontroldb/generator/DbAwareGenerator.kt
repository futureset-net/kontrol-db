...
abstract class DbAwareGenerator<T : ModelChange>(
    ...
    fun SqlString?.toQuoted(block: (String) -> String = { it }): String = this?.toQuoted(es)?.takeIf { it.isNotBlank() }?.let(block) ?: ""

    fun <T : SqlString> joinQuotableValues(
        items: Collection<T>,
        separateBy: String = ", ",
        block: (String) -> String = { it },
    ): String = items.joinToString(separator = separateBy, transform = { block(it.toQuoted()) })

    protected open fun convertSingle(): T.() -> String? = { null }

    override fun convert(change: T): List<String> = listOfNotNull(convertSingle().invoke(change))

    final override fun canApply(): Boolean = canApplyTo(es)

    protected fun Resource.text() = es.resourceResolver.resourceText(this)

    protected fun Resource.reader(): BufferedReader = es.resourceResolver.reader(this)

    open fun canApplyTo(es: EffectiveSettings): Boolean = true
}
